...
_secrets_get_qrcode() {
	_SECRET_TEMP_FILE=$(mktemp)
	_defer _secrets_cleanup_qrcode_tempfile

	if [ -z "$secret_value" ]; then
		_secrets_get_stdout | qrencode -o $_SECRET_TEMP_FILE
	else
		printf '%s\n' "$secret_value" | qrencode -o $_SECRET_TEMP_FILE
	fi

	_open $_SECRET_TEMP_FILE >/dev/null 2>&1
}

_secrets_cleanup_qrcode_tempfile() {
	rm -f $_SECRET_TEMP_FILE
}
